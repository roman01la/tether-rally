...
static void complete_frame(fpv_assembler_t *asm_, fpv_frame_assembly_t *frame)
{
    ...
    {
        if (frame->received_mask[i / 64] & (1ULL << (i % 64)))
        {
...
int fpv_assembler_add_fragment(fpv_assembler_t *asm_, const fpv_video_fragment_t *frag)
{
    ...
    if (frame->received_mask[frag->frag_index / 64] & (1ULL << (frag->frag_index % 64)))
    {
    ...
    frame->received_mask[frag->frag_index / 64] |= (1ULL << (frag->frag_index % 64));
