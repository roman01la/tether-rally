 ...
 * - MAX_INFLIGHT_FRAMES = 12 (sized for 720p with jitter)
 * - FRAME_TIMEOUT_MS = 80ms (~5 frames at 60fps)
...
/* Maximum fragments per frame (enough for ~300KB frame at 1200 byte payload) */
#define FPV_MAX_FRAGMENTS 256

/* Maximum AU data size */
#define FPV_MAX_AU_SIZE (512 * 1024) /* 512 KB */
...
typedef struct
{
    ...
    uint64_t received_mask[4]; /* Bitmap of received fragments (256 bits) */
...
typedef struct
{
    ...
    bool has_spspps;
    /* Timing telemetry */
    uint64_t first_packet_time_us; /* When first fragment arrived */
    uint64_t assembly_complete_us; /* When all fragments received */
} fpv_access_unit_t;
