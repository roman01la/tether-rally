...
static int create_session(fpv_decoder_t *dec)
{
    ...
    /* Output pixel format - BGRA for direct OpenGL upload without conversion */
    ...
    if (status != noErr)
    {
        fprintf(stderr, "Failed to create decompression session: %d\n", (int)status);
        return -1;
    }
    ...
    /* Disable frame reordering - we're using baseline profile (no B-frames) */
    VTSessionSetProperty(dec->session,
