...
static void process_video_fragment(app_ctx_t *ctx, const uint8_t *buf, size_t len)
{
    ...
    while ((au = fpv_assembler_get_au(&ctx->assembler)) != NULL)
    {
        ...
        if (decode_result == 0)
        {
            /* Update video time only on SUCCESSFUL decode */
...
static void send_periodic_messages(app_ctx_t *ctx)
{
    ...
    /* Request IDR if we haven't received video in 400ms (after initial connection) */
    if (ctx->got_first_frame && ctx->sender_known &&
        (now - ctx->last_video_time_us > 400000) && !ctx->idr_requested)
    {
        if (ctx->verbose)
        {
...
static void print_stats(app_ctx_t *ctx)
{
    ...
    /* Always print stats to diagnose issues */
...
int main(int argc, char *argv[])
{
    ...
    glfwSwapInterval(1); /* Enable VSync for smooth display-synced rendering */
    ...
    while (g_running && !glfwWindowShouldClose(ctx.window))
    {
        ...
        if (ctx.frame_slot.has_new_frame && ctx.frame_slot.pixbuf)
        {
            ...
            /* Upload texture (this is the only slow part, but doesn't block decode) */
            fpv_renderer_update_frame(ctx.renderer, &frame);
