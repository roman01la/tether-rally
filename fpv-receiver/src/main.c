...
static void print_stats(app_ctx_t *ctx)
{
    ...
    /* Print frame timing jitter */
    if (rnd_stats.avg_frame_interval_us > 0)
    {
        double target_interval = 1000000.0 / 60.0; /* 16667us for 60fps */
        double fps = 1000000.0 / rnd_stats.avg_frame_interval_us;
        printf("[JITTER] interval=%.1fms (%.1ffps) jitter=%.1fms (target=%.1fms)\n",
               rnd_stats.avg_frame_interval_us / 1000.0,
               fps,
               rnd_stats.frame_jitter_us / 1000.0,
