                                  ...
                                  const uint64_t *timing_us)
{
    ...
    if (r->last_frame_time_us > 0)
    {
        double interval_us = (double)(now - r->last_frame_time_us);

        if (r->stats.avg_frame_interval_us == 0)
        {
            r->stats.avg_frame_interval_us = interval_us;
            r->stats.frame_jitter_us = 0;
        }
        else
        {
            /* Compute jitter as EMA of absolute deviation from average */
            double deviation = interval_us - r->stats.avg_frame_interval_us;
            if (deviation < 0)
                deviation = -deviation;
            r->stats.frame_jitter_us = EMA_ALPHA * deviation + (1.0 - EMA_ALPHA) * r->stats.frame_jitter_us;
            r->stats.avg_frame_interval_us = EMA_ALPHA * interval_us + (1.0 - EMA_ALPHA) * r->stats.avg_frame_interval_us;
        }
    }
    r->last_frame_time_us = now;

    /* Update timing statistics if provided */
    if (timing_us)
    {
        uint64_t upload_end = now;
...
void fpv_renderer_draw(fpv_renderer_t *r, int viewport_width, int viewport_height)
{
    ...
    /* V coord: 0 at top, 1 at bottom (flipped for correct orientation) */
    float vertices[] = {
        -quad_w,
        quad_h,
        0.0f,
