...
static void on_encoded_frame(const fpv_rpicam_frame_t *frame, void *userdata)
{
    ...
    /* Frame pacing - ensure consistent intervals to avoid bursts.
     * At 60fps, target interval is 16667us. We pace if we're running
     * faster than 80% of target to absorb small timing variations. */
    uint64_t now = get_time_us();
    uint64_t target_interval_us = 1000000 / config.fps; /* 16667us for 60fps */
    if (g_last_send_time_us > 0)
    {
        uint64_t elapsed = now - g_last_send_time_us;
        uint64_t min_interval = (target_interval_us * 80) / 100; /* 80% of target */
        if (elapsed < min_interval)
        {
            usleep(target_interval_us - elapsed);
        }
    }
    g_last_send_time_us = get_time_us();
